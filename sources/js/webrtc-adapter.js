  ...
  shimPeerConnection: function shimPeerConnection() {
    ...
    window.RTCPeerConnection = function (config) {
      ...
      ['addEventListener', 'removeEventListener', 'dispatchEvent'].forEach(function (method) {
        this[method] = _eventTarget[method].bind(_eventTarget);
      });
    ...
    window.RTCPeerConnection.prototype._emitBufferedCandidates = function () {
      ...
      this._localIceCandidatesBuffer.forEach(function (event) {
        ...
        if (!event.candidate && this.iceGatheringState !== 'complete') {
          var complete = this.transceivers.every(function (transceiver) {
    ...
    window.RTCPeerConnection.prototype._createIceAndDtlsTransports = function (mid, sdpMLineIndex) {
      ...
      iceGatherer.onlocalcandidate = function (evt) {
        ...
        switch (this.iceGatheringState) {
          case 'new':
            this._localIceCandidatesBuffer.push(event);
            if (end && complete) {
              this._localIceCandidatesBuffer.push(new Event('icecandidate'));
            }
            break;
          case 'gathering':
            this._emitBufferedCandidates();
            this.dispatchEvent(event);
            if (this.onicecandidate !== null) {
              this.onicecandidate(event);
            }
            if (complete) {
              ...
              if (this.onicecandidate !== null) {
                this.onicecandidate(new Event('icecandidate'));
              }
              this.iceGatheringState = 'complete';
            }
            break;
      ...
      dtlsTransport.onerror = function () {
        // onerror does not set state to failed by itthis.
    ...
    window.RTCPeerConnection.prototype.setLocalDescription = function (description) {
      ...
      if (description.type === 'offer') {
        ...
        if (this._pendingOffer) {
          ...
          sections.forEach(function (mediaSection, sdpMLineIndex) {
            ...
            this._pendingOffer[sdpMLineIndex].localCapabilities = caps;
          });
        ...
        sections.forEach(function (mediaSection, sdpMLineIndex) {
          ...
          if (!rejected) {
            ...
            var params = this._getCommonCapabilities(localCapabilities, remoteCapabilities);

            // Start the RTCRtpSender. The RTCRtpReceiver for this
            // transceiver has already been started in setRemoteDescription.
            this._transceive(transceiver, params.codecs.length > 0, false);
          }
        });
      }
      ...
      p.then(function () {
        if (!hasCallback) {
          ...
          window.setTimeout(this._emitBufferedCandidates.bind(self), 500);
        }
      });
      return p;
    };
    ...
    window.RTCPeerConnection.prototype.setRemoteDescription = function (description) {
      ...
      sections.forEach(function (mediaSection, sdpMLineIndex) {
        ...
        if (description.type === 'offer' && !rejected) {
          var transports = this._createIceAndDtlsTransports(mid, sdpMLineIndex);
          if (isComplete) {
          ...
          if (this.localStreams.length > 0 && this.localStreams[0].getTracks().length >= sdpMLineIndex) {
            // FIXME: actually more complicated, needs to match types etc
            var localtrack = this.localStreams[0].getTracks()[sdpMLineIndex];
          ...
          // setLocalDescription.
          this._transceive(this.transceivers[sdpMLineIndex], false, direction === 'sendrecv' || direction === 'sendonly');
        } else if (description.type === 'answer' && !rejected) {
          transceiver = this.transceivers[sdpMLineIndex];
          ...
          this.transceivers[sdpMLineIndex].recvEncodingParameters = recvEncodingParameters;
          this.transceivers[sdpMLineIndex].remoteCapabilities = remoteCapabilities;
          this.transceivers[sdpMLineIndex].cname = cname;
          ...
          this._transceive(transceiver, direction === 'sendrecv' || direction === 'recvonly', direction === 'sendrecv' || direction === 'sendonly');
    ...
    window.RTCPeerConnection.prototype.createOffer = function () {
      ...
      tracks.forEach(function (mline, sdpMLineIndex) {
        ...
        sdp += SDPUtils.writeMediaSection(transceiver, transceiver.localCapabilities, 'offer', this.localStreams[0]);
      });
    ...
    window.RTCPeerConnection.prototype.createAnswer = function () {
      ...
      this.transceivers.forEach(function (transceiver) {
        // Calculate intersection of capabilities.
        var commonCapabilities = this._getCommonCapabilities(transceiver.localCapabilities, transceiver.remoteCapabilities);

        sdp += SDPUtils.writeMediaSection(transceiver, commonCapabilities, 'answer', this.localStreams[0]);
      });
